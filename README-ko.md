...
5.  **느림** : https://github.com/memoryInject/wsl-clipboard 이 프로젝트와 비교해서 월등히 빠름
...
> macOS 시스템에서 `man pbcopy` 명령을 입력했을 때 나타나는 모든 기능을 구현하는 것이 목표입니다.

**주요 기능 및 고려사항:**

-   **로케일(Locale) 환경 변수 참조:** `LANG=en_US.UTF-8`과 같은 로케일 환경 변수를 참조하여 입력 및 출력 인코딩을 결정할 것입니다.
-   **처리 가능 객체:**
    - [v]   **일반 텍스트 (Plain Text):** 기본적인 텍스트 복사/붙여넣기를 지원합니다.
    - []  **EPS (Encapsulated PostScript):** 이미지 형식인 EPS 데이터를 처리할 수 있도록 구현할 예정입니다.
    - []  **RTF (Rich Text Format):** 서식 있는 텍스트인 RTF 데이터를 처리할 수 있도록 구현할 예정입니다.
-   **개발 형태:** 현재는 별칭(alias) 형태로 시작하지만, 향후에는 일반 스크립트 파일 또는 실행파일로 변경될 가능성이 큽니다.
-   **개발 우선순위:** 처리량보다는 반응성을 우선으로 하여 개발할 예정입니다. 즉, 기능의 즉각적인 응답성과 사용자 경험을 중요하게 생각합니다.
